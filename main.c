        ...
        if (rank != root) { // sensor in 2d grid
            ...
            if (sensor_reading > SENSOR_THRESHOLD) {
                ...
                (left_reading > -1 && down_reading > -1) || (up_reading > -1 && down_reading > -1))  {
                    ...
                    MPI_Request req;
                    MPI_Isend(&report, 1, report_type, root, BASE_TAG, MPI_COMM_WORLD, &req);
            ...
            while (msg < size-1) {
                ...
                for (i = 0; i < size; i++) {
                    ...
                    if (flag) { // there exists a message
                        printf("base- ");
                        MPI_Recv(&report, 1, report_type, stat.MPI_SOURCE, BASE_TAG, MPI_COMM_WORLD, &stat);
                        printf("base-- from %d read %d msg %d time %s\n", stat.MPI_SOURCE, report.reading, report.num_msg, report.timestamp);
                        break;
                    }
                }
                msg++;
    ...
    while (msg < size-1) {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (flag) { // there exists a message
                ...
                printf("base-- from %d read %d time %s\n", stat.MPI_SOURCE, report.reading, report.timestamp);
                break;
            }
        }
        msg++;
