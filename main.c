...
void base_station(int rank, int size);
void sensor_node(int rank, int root, MPI_Comm comm, int coord[]);

typedef struct report_struct { // struct for node reports sent to base station
    int reading;
    float time_taken;
    char *timestamp;
...
int main(int argc, char *argv[]) {
    ...
    for (iteration = 0; iteration < NUM_ITERATIONS; iteration++) { // run NUM_ITERATIONS times
        ...
        if (rank != root) { // sensor in 2d grid
            sensor_node(rank, root, comm, coord);
        } else {
            base_station(root, size);
        }
    }
    MPI_Finalize();
    return; // exit
